...
public class CalibrationFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        Button updateCalib = (Button) rootView.findViewById(R.id.button_update_calib);
    ...
    private void extractValuesSeries() {
        ...
        for(int i=0; i < spectrogram.length; i++){
            data[i] =  meanExtraction(spectrogram[i], currentHalfCarrierWidth);
        }
    ...
    private Bitmap extractValues() {
        ...
        for(int i=0; i < spectrogram.length; i++){
            double[] valuePerTimestep = meanExtraction(spectrogram[i], currentHalfCarrierWidth);
    ...
    private void drawLine(int lastMean, int mean, int[][] image, int colNumber) {
        ...
        int dx = Math.abs(colNumber -x0), sx = x0< colNumber ? 1 : -1;
        int dy = -Math.abs(mean -y0), sy = y0< mean ? 1 : -1;
        int err = dx+dy, e2; /* error value e_xy */

        while(true){
            image[x0][y0] = Color.rgb(255, 0, 0);
            if (x0== colNumber && y0== mean) break;
            e2 = 2*err;
    ...
    private double[] meanExtraction(double[] values, int halfCarrierWidth) {
        double[] means = new double[2];

        //high doppler
        double meanWeightsHigh = 0.0;
        double meanHigh = 0.0;
        int offset = (int) Math.ceil(CalibrationFragment.CARRIER_IDX) + halfCarrierWidth;
        ...
        double meanLow = 0.0;
        offset = (int) Math.floor(CalibrationFragment.CARRIER_IDX) - halfCarrierWidth;
