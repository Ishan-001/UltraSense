...
public class SettingsFragment extends PreferenceFragmentCompat implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    private void togglePreferences() {
        ...
        addPreferencesFromResource(R.xml.preferences);
        assert prefMode != null;
        if(prefMode.equals(CW_MODE)){
            getPreferenceScreen().removePreference(Objects.requireNonNull(findPreference(FMCW_PARAS_KEY)));
        }
        else
            getPreferenceScreen().removePreference(Objects.requireNonNull(findPreference(CW_PARAS_KEY)));
    ...
    private void updateSummaries() {
        for(int i=0; i < getPreferenceScreen().getPreferenceCount(); i++){
            androidx.preference.Preference p = getPreferenceScreen().getPreference(i);
            updateSummaryForPreference(p);
        }
    }

    private void updateSummaryForPreference(Preference p){
        if(p instanceof androidx.preference.ListPreference){
